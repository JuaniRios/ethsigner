...
var waitForTalisman = async (retries = 3, delay = 250) => {
...
var detectWalletProviders = async () => {
  const providers = [];
...
var connectWallet = async (provider) => {
...
var signTypedData = async () => {
  ...
  try {
    ...
    const domainHash = exports_ethers.TypedDataEncoder.hashDomain(msgParams.domain);
    console.log("Domain Separator:", domainHash);
    const structHash = exports_ethers.TypedDataEncoder.hashStruct("ParticipationAuthorization", types, msgParams.message);
    console.log("Message Hash (hashStruct):", structHash);
    const concatenated = exports_ethers.concat([
      "0x1901",
      domainHash,
      structHash
    ]);
    console.log("Raw Concatenation (\x19\x01 || domainSeparator || hashStruct):", concatenated);
    const finalHash = exports_ethers.keccak256(concatenated);
...
//# debugId=47AE3483C4C8F47264756E2164756E21
